...
const COMPLETIONS = [
    ...
    // ── Messaging ─────────────────────────────────────────
    { n:'onMessage',         m:'event',          v:"onMessage('${1:messageName, d:'Listen for a named message. Fires when sendMessage or broadcastAll targets this object.'}', (data) => {\n  \n});" },
    // ── Mouse events ──────────────────────────────────────
    ...
    { n:'onStateEnter',      m:'event state',    v:"onStateEnter('${1:idle, d:'Fires when setState() switches to a named state. Gets (newState, prevState).'}', (newState, prevState) => {\n  playAnimation('${1:idle}');\n});" },
    { n:'onStateExit',       m:'event state',    v:"onStateExit('${1:attack, d:'Fires when leaving a state. Gets (oldState, nextState).'}', (oldState, nextState) => {\n  stopAnimation();\n});" },
    ...
    { n:'walkTo',            m:'navigate',  v:"walkTo(${1:5, d:'Pathfind to a world position. Avoids static obstacles. opts: { speed, avoidStatic }.'}, ${2:3}, { speed: ${3:4}, avoidStatic: true })" },
    { n:'walkToObject',      m:'navigate',  v:"walkToObject('${1:Player, d:'Pathfind toward another named object. opts: { speed, avoidStatic }.'}', { speed: ${2:4}, avoidStatic: true })" },
    { n:'stopWalking',       m:'navigate',  v:'stopWalking()', d:'Cancel any active pathfinding movement.'},
    { n:'moveTo',            m:'position',  v:'moveTo(${1:x, d:'Teleport to an exact world position (x, y).'}, ${2:y})' },
    { n:'move',              m:'position',  v:'move(${1:dx, d:'Move by an offset (dx, dy) in world units.'}, ${2:dy})' },
    { n:'moveForward',       m:'position',  v:'moveForward(${1:speed, d:'Move in the direction this object is currently facing at a given speed.'})' },
    { n:'lookAt',            m:'position',  v:'lookAt(${1:tx, d:'Rotate this object to face a world position (tx, ty).'}, ${2:ty})' },
    ...
    { n:'setVelocity',       m:'velocity',  v:'setVelocity(${1:vx, d:'Set both X and Y velocity at once in world units/sec.'}, ${2:vy})' },
    ...
    { n:'fadeIn',            m:'display',   v:'fadeIn(${1:duration, d:'Increase alpha toward 1 over duration seconds. Call inside onUpdate with dt.'}, dt)' },
    { n:'fadeOut',           m:'display',   v:'fadeOut(${1:duration, d:'Decrease alpha toward 0 over duration seconds. Call inside onUpdate with dt.'}, dt)' },
    // Tag / Group
    ...
    // Messaging
    { n:'sendMessage',       m:'message',   v:"sendMessage('${1:tag, d:'Send a message to all objects with a tag, or directly to a proxy. data is optional.'}', '${2:message}', ${3:data})" },
    { n:'sendMessage(proxy)', m:'message',   v:"sendMessage(${1:other, d:'Send a message directly to a proxy object (from find, collision, etc).'}, '${2:message}', ${3:data})" },
    { n:'broadcastGroup',    m:'message',   v:"broadcastGroup('${1:group, d:'Send a message to all objects in a group.'}', '${2:message}')" },
    ...
    { n:'other.sendMessage', m:'proxy',     v:"other.sendMessage('${1:msg, d:'Send a message directly to this specific proxy object.'}', ${2:data})" },
    { n:'other.clone',       m:'proxy',     v:'other.clone(${1:other.x, d:'Clone the referenced object at a given position.'}, ${2:other.y})' },
    ...
    // Scene
    { n:'gotoScene',         m:'scene',     v:"gotoScene('${1:SceneName, d:'Switch to another scene by name or index. Optional second arg is a transition.'}')" },
    ...
    { n:'drawText',          m:'text',       v:"drawText('${1:Hello, d:'Draw text in the game world. id keeps it persistent across frames. Returns a proxy.'}', ${2:0}, ${3:0}, { id: '${4:label1}', fontSize: ${5:32}, fill: '${6:#ffffff}' })" },
    ...
    // Camera
    { n:'cameraFollow',      m:'camera',    v:'cameraFollow(find("${1:Player, d:'Make the camera smoothly follow a proxy. smoothing = lerp speed (e.g. 6).'}"), ${2:6})' },
    { n:'cameraUnfollow',    m:'camera',    v:'cameraUnfollow()', d:'Stop the camera from following any object.'},
    { n:'cameraMoveTo',      m:'camera',    v:'cameraMoveTo(${1:x, d:'Move the camera to a specific world position instantly.'}, ${2:y})' },
    ...
    { n:'cameraShake',       m:'camera',    v:'cameraShake(${1:0.2, d:'Shake the camera. amplitude = strength, duration = seconds.'}, ${2:0.3})' },
    // Input
    ...
    { n:'onSwipe',           m:'mobile',    v:"onSwipe('${1:left, d:'Fires on a touch swipe. direction: "left"|"right"|"up"|"down"|"any".'}', () => {\n  \n});" },
    ...
    { n:'say duration',      m:'dialog',   v:"say('${1:Hello!, d:'Show a speech bubble for a specific number of seconds.'}', ${2:3})" },
    { n:'think',             m:'dialog',   v:"think('${1:Hmm..., d:'Show a thought bubble above this object. Optional duration in seconds.'}')" },
    { n:'think duration',    m:'dialog',   v:"think('${1:Hmm..., d:'Show a thought bubble for a specific number of seconds.'}', ${2:3})" },
    // Chat dialog
    { n:'showChat',          m:'dialog',   v:"showChat('${1:NPC, d:'Open the chat dialog. Callback receives player input, should return NPC reply.'}', (input) => {\n  if (input.includes('${2:hello}')) return '${3:Hey there!}';\n  return 'I don\'t understand.';\n})" },
    ...
    { n:'aiChat',            m:'AI dialog', v:"aiChat('${1:NPC Name, d:'Open an AI-powered chat using Claude. Provide NPC name, description, and API key.'}', '${2:You are ${1:NPC Name}, a character in a game. Reply in 1-2 sentences.}')" },
    // Physics — readable helper functions
    { n:'applyForce',            m:'physics (dynamic)',   v:'applyForce(${1:fx, d:'Apply a continuous force (world units/sec²) to a dynamic physics body.'}, ${2:fy})' },
    { n:'applyImpulse',          m:'physics (dynamic)',   v:'applyImpulse(${1:ix, d:'Apply an instant impulse to a dynamic body (like a single shove).'}, ${2:iy})' },
    { n:'setPhysicsVelocity',    m:'physics (dynamic)',   v:'setPhysicsVelocity(${1:vx, d:'Directly set the dynamic physics body\'s velocity. Dynamic bodies only.'}, ${2:vy})' },
    ...
    { n:'physics.setVelocity',         m:'physics (dynamic)',   v:'physics.setVelocity(${1:vx, d:'Directly set physics body velocity. Dynamic bodies only.'}, ${2:vy})' },
    { n:'physics.applyForce',          m:'physics (dynamic)',   v:'physics.applyForce(${1:fx, d:'Apply a force directly to this physics body.'}, ${2:fy})' },
    { n:'physics.applyImpulse',        m:'physics (dynamic)',   v:'physics.applyImpulse(${1:ix, d:'Apply an instant impulse directly to this physics body.'}, ${2:iy})' },
    ...
    { n:'GameSave.set',      m:'save',      v:"GameSave.set('${1:key, d:'Save a value permanently. GameSave.set("score", 100).'}', ${2:value})",          d:'Save a value permanently. GameSave.set("score", 100)' },
    { n:'GameSave.get',      m:'save',      v:"GameSave.get('${1:key, d:'Load a saved value. Second arg = default if key not found yet.'}', ${2:defaultValue})",   d:'Load a saved value. Second arg = default if not found yet' },
    ...
    { n:'GameSave.setAll',   m:'save',      v:"GameSave.setAll({ ${1:key, d:'Save multiple keys at once from a plain object.'}: ${2:value} })",     d:'Save multiple keys at once from a plain object' },
    { n:'GameSave.getAll',   m:'save',      v:'GameSave.getAll()',                              d:'Get all saved data as a plain object' },
    { n:'GameSave.increment',m:'save',      v:"GameSave.increment('${1:key, d:'Add to a numeric key (creates at 0 if missing). Returns new value.'}', ${2:1})",        d:'Add to a numeric key (creates at 0 if missing). Returns new value' },
    ...
    // Math
    { n:'lerp',              m:'math',      v:'lerp(${1:a, d:'Linear interpolate: lerp(a, b, t). t is clamped 0–1.'}, ${2:b}, ${3:t})' },
    { n:'clamp',             m:'math',      v:'clamp(${1:v, d:'Clamp v between lo and hi.'}, ${2:min}, ${3:max})' },
    { n:'dist',              m:'math',      v:'dist(${1:x1, d:'Euclidean distance between two world points.'}, ${2:y1}, ${3:x2}, ${4:y2})' },
    { n:'rand',              m:'math',      v:'rand(${1:min, d:'Random float in [min, max).'}, ${2:max})' },
    { n:'randInt',           m:'math',      v:'randInt(${1:min, d:'Random integer in [min, max] inclusive.'}, ${2:max})' },
    { n:'pick',              m:'math',      v:'pick([${1:a, d:'Pick a random element from an array.'}, ${2:b}, ${3:c}])' },
    ...
    { n:'mapRange',          m:'math',      v:'mapRange(${1:v, d:'Re-map v from range [a1,b1] into range [a2,b2].'}, ${2:a1}, ${3:b1}, ${4:a2}, ${5:b2})' },
    ...
    { n:'max',               m:'math',      v:'max(${1:a, d:'Return the larger of two values.'}, ${2:b})' },
    { n:'min',               m:'math',      v:'min(${1:a, d:'Return the smaller of two values.'}, ${2:b})' },
    // Game helpers
    { n:'addImpulse',        m:'game',     v:'addImpulse(${1:vx, d:'Apply a one-shot velocity kick (no physics body required).'}, ${2:vy})' },
    { n:'boundsClamp',       m:'game',     v:'boundsClamp(${1:0, d:'Clamp this object inside the world bounds. margin = padding in units.'})' },
    { n:'boundsClamp destroy',m:'game',    v:'boundsClamp(${1:0, d:'Destroy this object if it leaves the world bounds.'}, true)  // destroy if offscreen' },
    { n:'offScreen',         m:'game',     v:'offScreen(${1:0, d:'Returns true if this object is outside the visible game area.'})  // → true if offscreen' },
    { n:'spawnCopy',         m:'game',     v:"spawnCopy('${1:Name, d:'Spawn a copy of a named object at (x, y). Returns a proxy.'}', ${2:x}, ${3:y})" },
    { n:'trackTarget',       m:'game',     v:'trackTarget(${1:target, d:'Move this object toward a target proxy each frame at a given speed.'}, ${2:speed}, dt)' },
    { n:'hitFlash',          m:'game',     v:"hitFlash('${1:#ffffff, d:'Flash the sprite tint to a color then back. Good for damage feedback.'}', ${2:0.1})" },
    { n:'objectShake',       m:'game',     v:'objectShake(${1:0.2, d:'Shake this object in place. amplitude = strength, duration = seconds.'}, ${2:0.25})' },
    // Debug
    ...
    { n:'soundPlay opts',    m:'sound',    v:"soundPlay('${1:name, d:'Play a sound with options: loop, volume (0–1), range (spatial falloff).'}', { loop:${2:false}, volume:${3:1.0}, range:${4:400} })" },
    ...
    // Timer
    { n:'wait',              m:'timer',    v:'wait(${1:seconds, d:'Run a callback after a delay in seconds. Non-blocking (async-safe).'}, () => {\n  ${2:// code here}\n})' },
    // Physics control
    ...
    { n:'distanceTo pos',    m:'distance', v:'distanceTo(${1:x, d:'Distance from this object to a world position (x, y).'}, ${2:y})' },
    { n:'distanceTo obj',    m:'distance', v:'distanceTo(find("${1:label, d:'Distance from this object to a found object by name.'}"))' },
    // Tween
    { n:'tween',             m:'tween',    v:"tween({ ${1:alpha, d:'Smoothly animate a property to a target value over duration seconds.'}:${2:0} }, ${3:0.5}, '${4:easeOut}')" },
    { n:'tween complete',    m:'tween',    v:"tween({ ${1:x, d:'Tween with a callback that fires when the animation finishes.'}:${2:5} }, ${3:1}, '${4:linear}', () => {\n  ${5:// done}\n})" },
    // Repeat timers
    { n:'repeat',            m:'repeat',   v:'repeat(${1:1, d:'Run a callback every N seconds. Returns an id for cancelRepeat.'}, () => {\n  ${2:// code}\n})' },
    { n:'cancelRepeat',      m:'repeat',   v:'cancelRepeat(${1:id, d:'Cancel a repeat timer by the id returned from repeat().'})' },
    // Spawn
    { n:'spawnObject',       m:'spawn',    v:"spawnObject('${1:AssetName, d:'Spawn a fresh copy of an asset-library object at (x, y). Returns a proxy.'}', ${2:x}, ${3:y})" },
    { n:'spawnObject cb',    m:'spawn',    v:"spawnObject('${1:AssetName, d:'Spawn with a callback to configure the new object immediately.'}', ${2:x}, ${3:y}, (obj) => {\n  ${4:// obj.velocityX = 10;}\n})" },
    { n:'cloneSelf',         m:'clone',    v:"cloneSelf(${1:getX(), d:'Clone this exact object (sprite, physics, script) at (x, y). Returns a proxy.'}, ${2:getY()})" },
    { n:'cloneSelf cb',      m:'clone',    v:"cloneSelf(${1:getX(), d:'Clone with a callback. Use c.opts to pass data to the clone.'}, ${2:getY()}, (c) => {\n  ${3:c.velocityX = 3;}\n})" },
    { n:'cloneObject',       m:'clone',    v:"cloneObject('${1:Name, d:'Clone any object by name at (x, y). Copies all its properties.'}', ${2:x}, ${3:y})" },
    { n:'cloneObject cb',    m:'clone',    v:"cloneObject('${1:Name, d:'Clone any object with a callback to configure it on spawn.'}', ${2:x}, ${3:y}, (c) => {\n  ${4:c.velocityX = 3;}\n})" },
    // Raycast
    { n:'raycast',           m:'raycast',  v:'raycast(${1:x1, d:'Cast a ray from (x1,y1) to (x2,y2). Returns the first hit, or null.'}, ${2:y1}, ${3:x2}, ${4:y2})' },
    { n:'raycast tag',       m:'raycast',  v:"raycast(${1:x1, d:'Raycast filtered to only hit objects with a specific tag.'}, ${2:y1}, ${3:x2}, ${4:y2}, '${5:enemy}')" },
    // Radius query
    { n:'getObjectsInRadius',m:'radius',   v:'getObjectsInRadius(${1:cx, d:'Returns all objects within radius of (cx, cy). Optional tag filter.'}, ${2:cy}, ${3:radius})' },
    // Z-order
    ...
    { n:'screenToWorld',     m:'coords',   v:'screenToWorld(${1:sx, d:'Convert screen pixel coordinates to world coordinates. Returns { x, y }.'}, ${2:sy})' },
    { n:'worldToScreen',     m:'coords',   v:'worldToScreen(${1:wx, d:'Convert world coordinates to screen pixel coordinates. Returns { x, y }.'}, ${2:wy})' },
    // Key event handlers
    { n:'onKeyDown',         m:'key event',v:"onKeyDown('${1:arrowleft, d:'Fires once when a specific key is pressed down.'}', () => {\n  ${2:// code}\n})" },
    { n:'onKeyUp',           m:'key event',v:"onKeyUp('${1:arrowleft, d:'Fires once when a specific key is released.'}', () => {\n  ${2:// code}\n})" },
    // Physics helpers
    { n:'setGravityScale',  m:'physics',  v:'setGravityScale(${1:1.0, d:'Multiply gravity for this object. 0 = floats, 2 = double gravity.'})' },
    // Extra math
    { n:'smoothstep',        m:'math',     v:'smoothstep(${1:lo, d:'S-curve blend between lo and hi. Smoother than lerp at the edges.'}, ${2:hi}, ${3:x})' },
    { n:'normalize',         m:'math',     v:'normalize(${1:vx, d:'Normalize vector (vx, vy) to unit length. Returns { x, y }.'}, ${2:vy})' },
    { n:'angleTo',           m:'math',     v:'angleTo(${1:x1, d:'Angle in degrees from (x1,y1) to (x2,y2).'}, ${2:y1}, ${3:x2}, ${4:y2})' },
    // Debug draw
    { n:'drawDebugLine',     m:'debug draw',v:'drawDebugLine(${1:x1, d:'Draw a line in world space for debugging. Color and duration are optional.'}, ${2:y1}, ${3:x2}, ${4:y2})' },
    { n:'drawDebugLine opts',m:'debug draw',v:"drawDebugLine(${1:x1, d:'Draw a debug line with explicit color (hex) and duration (seconds).'}, ${2:y1}, ${3:x2}, ${4:y2}, '${5:#ff0000}', ${6:0.5})" },
    { n:'drawDebugCircle',   m:'debug draw',v:'drawDebugCircle(${1:cx, d:'Draw a circle in world space for debugging.'}, ${2:cy}, ${3:radius})' },
    // Scene transitions
    { n:'gotoScene fade',    m:'scene',    v:"gotoScene('${1:Level2, d:'Switch to another scene with a fade transition.'}', 'fade')" },
    { n:'gotoScene slide',   m:'scene',    v:"gotoScene('${1:Level2, d:'Switch to another scene with a slide-left transition.'}', 'slide-left')" },
    ...
    { n:'takeDamage src',    m:'health',    v:'takeDamage(${1:10, d:'Deal damage and pass the source object so onDamage knows who hit.'}, other)' },
    ...
    { n:'opts set (clone)',  m:'clone opts',v:"cloneSelf(getX(), getY(), (c) => {\n  c.opts.${1:speed, d:'Pass custom data to a clone via c.opts before onCloneStart fires.'} = ${2:5};\n  c.opts.${3:damage} = ${4:1};\n});" },
    { n:'opts read (clone)', m:'clone opts',v:"onCloneStart(() => {\n  // read opts set by spawner\n  velocityX = opts.${1:speed, d:'Read opts data in onCloneStart that was set by the spawner.'};\n});" },
    ...
    { n:'cloneInPlace',      m:'clone',    v:"cloneInPlace((c) => {\n  ${1:c.velocityX = 2;, d:'Clone this object at its current position.'}\n})" },

    // ── Screen / Proximity helpers ─────────────────────────
    { n:'inRangeOf',         m:'proximity',v:'inRangeOf(find("${1:Player, d:'Returns true if this object is within radius world units of a proxy.'}"), ${2:3})' },
    { n:'onceAfter',         m:'timer',    v:'onceAfter(${1:2, d:'Run a callback once after delay seconds (like wait, more explicit name).'}, () => {\n  ${2:destroySelf();}\n})' },

    // ── Raycast (extended) ─────────────────────────────────
    { n:'raycastAll',        m:'raycast',  v:'raycastAll(${1:x1, d:'Like raycast but returns all hits as an array.'}, ${2:y1}, ${3:x2}, ${4:y2})' },
    { n:'raycastFromSelf',   m:'raycast',  v:'raycastFromSelf(${1:0, d:'Cast a ray from this object\'s center at angleDeg for a given distance.'}, ${2:8})' },
    { n:'raycastFromSelf tag',m:'raycast', v:"raycastFromSelf(${1:0, d:'Raycast from self, filtered to a specific tag.'}, ${2:8}, '${3:wall}')" },
    { n:'hit.distance',      m:'raycast',  v:'${1:hit, d:'Distance from the ray origin to the hit point.'}.distance' },
    { n:'hit.point.x',       m:'raycast',  v:'${1:hit, d:'World X coordinate where the ray hit.'}.point.x' },
    { n:'hit.normal.x',      m:'raycast',  v:'${1:hit, d:'Surface normal X at the hit point. Use to deflect projectiles.'}.normal.x' },
    ...
    // ── AI Navigation ─────────────────────────────────────
    { n:'pursue',            m:'AI nav',   v:"pursue('${1:player, d:'Move toward a target tag using pathfinding. opts: { speed }.'}', { speed: ${2:3} })" },
    { n:'flee',              m:'AI nav',   v:"flee('${1:player, d:'Move away from a target tag. opts: { speed }.'}', { speed: ${2:4} })" },
    { n:'wander',            m:'AI nav',   v:'wander({ speed: ${1:1.5, d:'Roam randomly around the scene. opts: { speed, radius }.'}, radius: ${2:3} })' },
    { n:'canSee',            m:'AI nav',   v:"canSee('${1:player, d:'True if there is a clear line of sight to the target.'}')" },
    { n:'canSee opts',       m:'AI nav',   v:"canSee('${1:player, d:'canSee with a maximum range limit in world units.'}', { maxRange: ${2:8} })" },
    { n:'lastKnownPos',      m:'AI nav',   v:"lastKnownPos('${1:player, d:'Returns the last seen position { x, y } of a target, or null.'}')" },
    { n:'inFOV',             m:'AI nav',   v:"inFOV('${1:player, d:'True if target is within a field-of-view cone. Args: tag, degrees, range.'}', ${2:90}, ${3:6})" },
    ...
    // ── Drag (full set) ────────────────────────────────────
    { n:'makeDraggable',        m:'drag',  v:'makeDraggable()', d:'Let the player drag this object with mouse or touch.'},
    { n:'makeDraggable opts',   m:'drag',  v:"makeDraggable({ smooth:${1:16, d:'Draggable with options: smooth (lerp speed), clamp (bounds), scale (pop effect).'}, clamp:${2:true}, scale:${3:1.1} })" },
    ...
    { n:'makeThrowable',        m:'throw', v:'makeThrowable()', d:'Let the player fling this object with velocity on release.'},
    { n:'makeThrowable opts',   m:'throw', v:"makeThrowable({ speed:${1:1, d:'Throwable with options: speed multiplier, maxSpeed cap, scale on grab.'}, maxSpeed:${2:25}, clamp:${3:false}, scale:${4:1.08} })" },
    { n:'makeThrowable onThrow',m:'throw', v:"makeThrowable({ speed:${1:1, d:'Throwable with a callback that fires with (vx, vy) when the object is released.'}, onThrow:(vx,vy) => { ${2:log(vx,vy)} } })" },
    { n:'throwObject',          m:'throw', v:"throwObject(find('${1:Ball, d:'Throw another object using the current drag velocity.'}'))" },
    { n:'throwObject opts',     m:'throw', v:"throwObject(find('${1:Ball, d:'Throw another object with custom speed and maxSpeed options.'}'), { speed:${2:1.5}, maxSpeed:${3:30} })" },

    // ── Messaging (extended) ──────────────────────────────
    { n:'broadcastAll data',m:'message',v:"broadcastAll('${1:msg, d:'Send a message with a data payload to every scripted object in the scene.'}', ${2:data})" },
    ...
    { n:'wrap',              m:'math',      v:'wrap(${1:v, d:'Wrap v within [lo, hi]. Useful for looping angles.'}, ${2:lo}, ${3:hi})' },
    { n:'tan',               m:'math',      v:'tan(${1:a, d:'Tangent of an angle in radians.'})' },
    { n:'atan2',             m:'math',      v:'atan2(${1:y, d:'Angle in radians from the positive X axis to point (y, x).'}, ${2:x})' },
    { n:'pow',               m:'math',      v:'pow(${1:base, d:'Raise base to an exponent: pow(2, 10) = 1024.'}, ${2:exp})' },
    ...
    { n:'forever',           m:'loop',     v:"forever((dt) => {\n  ${1:// runs every frame, d:'Run a callback every frame with dt. Register inside onStart.'}\n});" },
    { n:'forever move',      m:'loop',     v:"forever((dt) => {\n  x -= ${1:3, d:'Example: move this object left every frame using dt.'} * dt;  // move left\n});" },

    // ── Clone return-value: var c = cloneSelf(...) ─────────
    { n:'cloneSelf assign',  m:'clone',    v:"var ${1:c, d:'Store the cloned object in a variable to control it later.'} = cloneSelf(${2:getX()}, ${3:getY()});" },
    { n:'cloneObject assign',m:'clone',    v:"var ${1:c, d:'Store a cloneObject result in a variable.'} = cloneObject('${2:Name}', ${3:x}, ${4:y});" },
    { n:'cloneInPlace assign',m:'clone',   v:"var ${1:c, d:'Store the in-place clone in a variable.'} = cloneInPlace();" },
    { n:'spawnObject assign', m:'spawn',   v:"var ${1:obj, d:'Store a spawned object in a variable.'} = spawnObject('${2:Name}', ${3:x}, ${4:y});" },

    // ── Proxy properties on a stored clone/object reference ─
    { n:'c.x',               m:'clone prop', v:'${1:c, d:'Read or set the X position of a stored object reference.'}.x' },
    { n:'c.y',               m:'clone prop', v:'${1:c, d:'Read or set the Y position of a stored object reference.'}.y' },
    { n:'c.rotation',        m:'clone prop', v:'${1:c, d:'Read or set rotation (degrees) of a stored reference.'}.rotation' },
    { n:'c.scaleX',          m:'clone prop', v:'${1:c, d:'Read or set horizontal scale of a stored reference.'}.scaleX' },
    { n:'c.scaleY',          m:'clone prop', v:'${1:c, d:'Read or set vertical scale of a stored reference.'}.scaleY' },
    { n:'c.velocityX',       m:'clone prop', v:'${1:c, d:'Read or set velocityX on a stored clone or spawned object.'}.velocityX' },
    { n:'c.velocityY',       m:'clone prop', v:'${1:c, d:'Read or set velocityY on a stored clone or spawned object.'}.velocityY' },
    { n:'c.alpha',           m:'clone prop', v:'${1:c, d:'Read or set opacity (0–1) of a stored reference.'}.alpha' },
    { n:'c.visible',         m:'clone prop', v:'${1:c, d:'Read or set visibility of a stored reference.'}.visible' },
    { n:'c.health',          m:'clone prop', v:'${1:c, d:'Read the current HP of a stored reference.'}.health' },
    { n:'c.state',           m:'clone prop', v:'${1:c, d:'Read the current state string of a stored reference.'}.state' },
    { n:'c.opts',            m:'clone prop', v:'${1:c, d:'Access the opts data bag on a stored clone reference.'}.opts.${2:myVar}' },
    { n:'c.destroy',         m:'clone prop', v:'${1:c, d:'Destroy a stored clone or object.'}.destroy()' },
    { n:'c.setVelocity',     m:'clone prop', v:'${1:c, d:'Set both velocity components on a stored reference.'}.setVelocity(${2:vx}, ${3:vy})' },
    { n:'c.stopMovement',    m:'clone prop', v:'${1:c, d:'Zero the velocity of a stored reference.'}.stopMovement()' },
    { n:'c.applyForce',      m:'proxy',      v:'${1:c, d:'Apply a physics force to a stored dynamic object.'}.applyForce(${2:fx}, ${3:fy})' },
    { n:'c.sendMessage',     m:'clone prop', v:"${1:c, d:'Send a message directly to a stored object.'}.sendMessage('${2:msg}', ${3:data})" },
    { n:'c.takeDamage',      m:'clone prop', v:'${1:c, d:'Deal damage to a stored object.'}.takeDamage(${2:10})' },
    { n:'c.isTop (opts)',    m:'clone prop', v:'${1:c, d:'Read a custom boolean flag from a clone\'s opts.'}.opts.isTop' },
    { n:'c.isTop set',       m:'clone prop', v:'${1:c, d:'Set a custom boolean flag on a clone via its opts.'}.opts.isTop = ${2:true}' },
    ...
    { n:'c.applyImpulse',       m:'proxy',     v:'${1:c, d:'Apply an instant impulse to a stored dynamic object.'}.applyImpulse(${2:ix}, ${3:iy})' },
    { n:'c.ammo',               m:'proxy',     v:'${1:c, d:'Read the ammo count of a stored reference.'}.ammo' },
    { n:'c.getState',           m:'proxy',     v:'${1:c, d:'Read the current state of a stored reference.'}.getState()' },
    { n:'c.setState',           m:'proxy',     v:"${1:c, d:'Set the state of a stored reference.'}.setState('${2:idle}')" },
    { n:'c.getHealth',          m:'proxy',     v:'${1:c, d:'Read the HP of a stored reference.'}.getHealth()' },
    { n:'c.setHealth',          m:'proxy',     v:'${1:c, d:'Set the HP of a stored reference.'}.setHealth(${2:100})' },
    { n:'c.isDead',             m:'proxy',     v:'${1:c, d:'True if a stored reference\'s HP is 0 or below.'}.isDead' },
    { n:'c.isInvincible',       m:'proxy',     v:'${1:c, d:'True if a stored reference is currently invincible.'}.isInvincible()' },

    // ── Raycast hit properties (flat — no ._rayHit needed) ────────────────
    { n:'hit.sprite',         m:'raycast',  v:'${1:hit, d:'The PIXI sprite of the hit object.'}.sprite' },
    { n:'hit.isTile',         m:'raycast',  v:'${1:hit, d:'True if the ray hit a tilemap tile instead of a sprite.'}.isTile' },
    { n:'hit.point.y',        m:'raycast',  v:'${1:hit, d:'World Y coordinate where the ray hit.'}.point.y' },
    { n:'hit.normal.y',       m:'raycast',  v:'${1:hit, d:'Surface normal Y at the hit point.'}.normal.y' },
    { n:'hit.tile',           m:'raycast',  v:'${1:hit, d:'Tile data object if the hit was a tile.'}.tile' },
    // ── drawText proxy methods ──────────────────────────────────────────────
    { n:'t.text',             m:'text',     v:'${1:t, d:'Directly set the text string on a drawText proxy.'}.text = ${2:value}' },
    { n:'t.setText',          m:'text',     v:"${1:t, d:'Update the displayed text on a drawText proxy.'}.setText(${2:'hello'})" },
    { n:'t.setTextStyle',     m:'text',     v:"${1:t, d:'Update font size or color on a drawText proxy.'}.setTextStyle({ fontSize: ${2:32}, fill: '${3:#fff}' })" },
    { n:'t.visible',          m:'text',     v:'${1:t, d:'Show or hide a drawText element.'}.visible = ${2:true}' },
    { n:'t.destroy',          m:'text',     v:'${1:t, d:'Remove this drawText element from the game.'}.destroy()' },
    // ── Text object (placed in editor) via find() ──────────────────────────
    { n:'setText (obj)',      m:'text',     v:"find('${1:TextLabel, d:'Update text on a named TextLabel object placed in the editor.'}').setText('${2:hello}')" },
